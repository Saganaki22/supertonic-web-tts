...
export class UnicodeProcessor {
  ...
  getTextMask(textIdsLengths) {
    return this.lengthToMask(textIdsLengths);
  }
...
export class TextToSpeech {
  ...
  async call(text, lang, style, totalStep, speed = 1.05, silenceDuration = 0.3, progressCallback = null) {
    if (style.ttl.dims[0] !== 1) throw new Error('Single speaker only supports single style');
...
export function writeWavFile(audioData, sampleRate) {
  ...
  for (let i = 0; i < audioData.length; i++) int16Data[i] = Math.floor(Math.max(-1.0, Math.min(1.0, audioData[i])) * 32767);
...
export async function loadVoiceStyle(voiceStylePaths) {
  ...
  const ttlDim1 = ttlDims[1], ttlDim2 = ttlDims[2];
  const dpDim1 = dpDims[1], dpDim2 = dpDims[2];
...
export async function loadTextToSpeech(onnxDir, sessionOptions = {}, progressCallback = null) {
  ...
  for (let i = 0; i < models.length; i++) {
    if (progressCallback) progressCallback(models[i].name, i + 1, models.length);
    sessions.push(await loadOnnx(models[i].path, sessionOptions));
  }
